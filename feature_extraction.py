...
if __name__ == "__main__":
    '''
    1.环境检查与初始化:
    获取当前工作目录名 (dname)，并检查是否是有效的CERT数据集文件夹名称（如 'r4.1', 'r4.2' 等）。如果不是，则抛出异常。
    创建三个临时文件夹用于存放中间数据：tmp, ExtractedData, DataByWeek, NumDataByWeek。
    '''
    ...
    '''
    2.配置参数:
    subsession_mode: 定义了子会话的划分方式。这里配置了按活动数量 (nact) 划分为25或50个活动一组，以及按时间 (time) 划分为120分钟或240分钟一段。如果不需要子会话，可以设置为空字典。
    numCores: 设置并行处理时使用的CPU核心数，默认为8。可以通过命令行参数传入来自定义。
    numWeek: 根据数据集名称确定总的周数（r4.x为73周，其他为75周）。
    '''
    ...
    '''
    1. 按周合并数据 (combine_by_timerange_pandas):
    调用 combine_by_timerange_pandas(dname) 函数。
    此函数读取原始的各种活动日志文件（如 http.csv, email.csv 等），并将它们按周合并。
    合并后的每周数据以 pickle 格式保存在 DataByWeek/ 文件夹下（例如 DataByWeek/0.pickle, DataByWeek/1.pickle ...）。
    '''
    ...
    '''
    2. 获取用户列表 (get_mal_userdata):
    调用 get_mal_userdata(dname) 函数。
    此函数读取合并后的每周数据，并提取出用户列表。
    用户列表以 pickle 格式保存在 tmp/ 文件夹下（例如 tmp/users.pickle）。
    '''
    ...
    '''
    3. 将每个活动转换为数值数据 (process_week_num):
    使用 Parallel 并行处理每个周的数据。
    每个周的数据通过 process_week_num(i, users, data=dname) 函数进行处理。
    处理后的数据以 pickle 格式保存在 NumDataByWeek/ 文件夹下（例如 NumDataByWeek/0.pickle, NumDataByWeek/1.pickle ...）。
    '''
    ...
    '''
    4. 提取数据到CSV (to_csv):
    使用 Parallel 并行处理每个模式（周/日/会话）的数据。
    每个模式的数据通过 to_csv(i, mode, dname, ul, uf_dict, list_uf, subsession_mode) 函数进行处理。
    处理后的数据以 CSV 格式保存在 ExtractedData/ 文件夹下（例如 ExtractedData/week.csv, ExtractedData/day.csv, ExtractedData/session.csv）。
    to_csv 函数会根据指定的 mode 从 NumDataByWeek/ 中读取数据，计算更高级别的特征（如统计特征、时间分布特征等），并将结果临时保存在 tmp/ 文件夹下的 pickle 文件中。
    ...
    for mode in ['week','day','session']:
        ...
        #     调用 get_u_features_dicts(users, data=dname) 获取用户特征的字典和列表。
        (ul, uf_dict, list_uf) = get_u_features_dicts(users, data= dname)
        # 再次使用 joblib.Parallel 并行处理，
        # 对 weekRange 中的每一周调用 to_csv(i, mode, dname, ul, uf_dict, list_uf, subsession_mode) 函数。
        ...
        for w in weekRange[1:]:
            # 然后，将 tmp/ 文件夹中对应 mode 和周的所有 pickle 文件合并，
            # 追加写入到 ExtractedData/ 文件夹下的一个总CSV文件中（文件名格式如 ExtractedData/weekr4.2.csv）。
            towrite = pd.read_pickle("tmp/"+str(w) + mode+".pickle")        
            towrite.to_csv(all_csv,header=False, index = False)
        
        # 如果 mode 是 'session' 并且 subsession_mode 不为空，还会对子会话进行类似的处理和CSV文件生成。
        if mode == 'session' and len(subsession_mode) > 0:
            for k1 in subsession_mode:
                for k2 in subsession_mode[k1]:
